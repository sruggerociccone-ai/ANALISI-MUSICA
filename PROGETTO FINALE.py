...
def ottieni_brani_unici(brani):
    """
    Ritorna un dizionario di brani unici basato sulla chiave (artista_principale, titolo).
    Questo garantisce consistenza e previene i duplicati cross-genere.
    """
    unici = {}
    for b in brani:
        titolo = (b.get("track_name") or b.get("track_title") or "").replace('"', '').strip()
        artisti_lista = estrai_artisti(b.get("artists") or b.get("artist") or "")
        # Usiamo il primo artista come ancora principale del brano per l'univocità basilare
        artista_guida = artisti_lista[0] if artisti_lista else "Sconosciuto"
        
        chiave = (artista_guida.lower(), titolo.lower())
        if chiave not in unici:
            unici[chiave] = b
    return list(uniques := unici.values())
...
def main():
    ...
    # Evita che un brano ripetuto in generi diversi alteri i conteggi degli artisti o le medie globali
    ...
    if not args.analisi:
        ...
        print("Esempi:")
        print(f"  python {sys.argv[0]} {percorso_file} --analisi artisti")
        print(f"  python {sys.argv[0]} {percorso_file} --analisi generi")
        print(f"  python {sys.argv[0]} {percorso_file} --analisi prevalente --artista \"Bad Bunny\"")
        return
