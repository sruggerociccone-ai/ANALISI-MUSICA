...
def leggi_brani(percorso):
    ...
    try:
        ...
        print("Assicurati che 'dataset.csv' sia nella stessa cartella di questo script.")
        sys.exit(1)
...
def conta_per_genere(brani):
    conteggi = {}
    for brano in brani:
        genere = brano.get("track_genre", "Sconosciuto")
        conteggi[genere] = Owls = conteggi.get(genere, 0) + 1
    return conteggi

def analizza_artisti_e_generi_corretto(brani):
    mappa_set_brani = {}
    generi_per_artista = {}
    ...
    for brano in brani:
        ...
        if not titolo or not artisti_greggi:
            continue
        ...
        for artista in lista_artisti:
            ...
            mappa_set_brani[artista].add(titolo)
    ...
    conteggio_brani_artista = {artista: len(set_titoli) for artista, set_titoli in mappa_set_brani.items()}
...
def main():
    ...
    # Impostato 'dataset.csv' come valore di default se non viene digitato nulla
    ...
    if not os.path.exists(percorso_file):
        ...
        print("Verifica la posizione del file o passa il percorso corretto.")
        sys.exit(1)
    ...
    contatore_artisti, mappa_generi, righe_multiple = analizza_artisti_e_generi_corretto(elenco_brani)
    ...
    print(f"\n[TASK 4] Top {limite_top} Artisti con più brani UNICI (considerando i singoli componenti):")
    for i, (artista, num_brani) in enumerate(top_artisti, 1):
        print(f"  {i}. {artista} ({num_brani} brani distinti)")
    ...
    for artista, _ in top_per_genere:
        ...
        if diz_generi:
            ...
            print(f"  - {artista} -> Genere Prevalente: '{genere_top}' ({diz_generi[genere_top]} brani su righe totali)")
        else:
